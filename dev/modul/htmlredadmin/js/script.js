...
$(document).on("click",".next_step",function(){
    ...
    if(page!='---' && page!=''){
      $.ajax({
        url: '/htmlredadmin/next_tmp.ajax', 
        type: 'POST',
        ...
        success: function(data){
            ...
            $.ajax({
                url: '/htmlredadmin/console_step.ajax', 
                type: 'POST',
                success: function(data){
...
$(document).on("click",".clear_step",function(){
    ...
    if(page!='---' && page!=''){
      $.ajax({
        url: '/htmlredadmin/clear_tmp.ajax', 
        type: 'POST',
        data: {page:page},
        success: function(data){
...
$(document).on("click",".prev_step",function(){
    ...
    if(page!='---' && page!=''){
      $.ajax({
        url: '/htmlredadmin/prev_tmp.ajax', 
        type: 'POST',
...
$(document).on("mousedown",".pthoto-block img,.listv,.dels,.copy3,.vz,.vz2,.save_red_kod,.save_attr_red,.okno_znak2 div,.copy2,.sozd .drops,.obj_up,.obj_down",function(){
    ...
    if(page!='---' && page!=''){
     $.ajax({
        url: '/htmlredadmin/save_tmp.ajax', 
        type: 'POST',
...
data.append('path',path);
    $.ajax({
        url: '/htmlredadmin/ajaxfiles_all.ajax', 
        type: 'POST',
        data: data,
        cache: false,
        dataType: 'json',
...
var val = $('.shs').val();
file='/modulis/htmlredadmin/sh/'+val+'.html';
$.ajax({ 
            type: "POST",   
            url: "/htmlredadmin/removefile.ajax",
            data: {file:file},
             success: function(data){  
...
if(name!='---'){
 $.ajax({
            type: "POST",
            url: "/htmlredadmin/stranicahtml.ajax",
            data: {name:name},
             success: function(data){
...
$(this).css('border','0px solid #eee');
 ...
 $(document).on("click",".saves",function(){
  ...
  var nameadmin = $('.adress').val();
  var adminstr=$('.site_str').val();
  if(nameadmin!='' || adminstr!='---'){      
      if($('.rem').prop('checked')){
          var rem='yes';
      }
      else{
      ...
      $.ajax({
        type: "POST",
        url: "/htmlredadmin/files.ajax",
        data: {admins:name,font:font,nameadmin:nameadmin,komment:komment,rem:rem},
        success: function(data){
...
$(document).on("click",".delete_str",function(){
  ...
  var file = $('.site_str').val(); 
   $.ajax({
      type: "POST",
      url: "/htmlredadmin/del_files_sh.ajax",
      data: {file:file},
      success: function(data){
        alert('Страница удалена');
        document.location.href='/htmlredadmin.modul';
