...
<div class='head_admin'>
    <div class='center'><a href='/workarea/'><img src="/system/admin-inc/img/logo.png" class="logo-fv">Interface CRISTAL</a>
    </div>
    ...
    <form  method='post' class='exits'>
        <input type='submit' name='Foton-key' value='exit' id='exitst'>
    </form>
</div>
                    ...
                    <div class='contener'>
                        ...
                        foreach ($controller_class->list_w($key) as $arr) {
                            foreach ($arr as $key2 => $val2) {
                                ?>
                                <a href='/<?= $GLOBALS['foton_setting']['interface']; ?>/<?= $key; ?>/<?= $key2; ?>'
                                   target="CONTENT"><?= $val2; ?></a>
                                <?
                            }
                        }
                        ?>
...
<div>
 ...
 <select id="patch3" name="shablon" patchs="view" dir="<?= $GLOBALS['foton_setting']['sitedir']; ?>"><option
             value='---'>Страницы сайта</option>
...
<input type="button" value="NEW VIEW" id="newkod2">
    ...
    <div class="okno-glob input-wrapper" data-title='Перетаскивание при двойном нажатии'>
        ...
        <form action="" method="post" class="kodes"><p class="x"></p><span class="kod55"> </span>
            ...
            <span class="kod5"><p> <input type="text" name="scriptname" id="scriptname" disabled><input type="hidden"
                                                                                                        name="scriptdir"
                                                                                                        id="scriptdir"><input
                            type="hidden" name="scriptpath" id="scriptpath"> </p></span><span id="form"><textarea
                        id="scripti-glob" name="script">
    ...
    <div class='oknologi'>
        <input type='text' class='search' placeholder='Поиск...'><input type='number' class='chislo' placeholder='число'
                                                                        value='<?= date("j"); ?>'><select class='mes'>
...
<textarea class='textlog' id=" textarea-example
            "></textarea>
...
<textarea class="text1 text1_inc"
          placeholder="1 буфер стрелка вверх вниз очитска F7"><? if (isset($_SESSION['text1'])) {
        echo $_SESSION['text1'];
    } ?></textarea>
<textarea class="text2 text1_inc"
          placeholder="2 буфер стрелка вправо влево очитска F7"><? if (isset($_SESSION['text2'])) {
        echo $_SESSION['text2'];
    } ?></textarea>
