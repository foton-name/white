...
if(isset($data['update']) && $data['update']!==false){?>
        ...
        <form action='' enctype='multipart/form-data' method='post' class="short_table_inc_one forminc">
           ...
           ?>
         <br><br>
       <input type="submit" name="delete-insert-include" value="Удалить"><input type="submit" class='id-up'  value="Обновить"><br>
        </form>
...
<form action='' method='post' id="i_f_filtr">
    <? foreach($data['filter_find'][0] as $key_i=>$f_item){ ?>
     <?$f_item=str_replace("name='","name='find_",$f_item); echo $f_item;?>
    <?}?>
...
<div  class="short_table_inc sort_bx">
     <?  foreach($data['filter_sort'][0] as $key_i=>$s_item){ if($s_item!=''){?>
...
<?php  echo $data['pagin']; ?>
    ...
    <div class='form-red-inc'>
            ...
            <form action='' enctype='multipart/form-data' method='post'>
               ...
               ?>
                 <br><br>
               <input type="submit" name="delete-insert-include" value="Удалить"><input type="submit" class='id-up-one'  value="Обновить"><br>
            </form>
        <?}?>
        </div>
        <?
