...
if($val['grafik']=='1'){
?><div class="grafik_f">
<canvas id="countries<?=$val['grafik'].$key2;?>" width="800" height="300">
       <script>
            var countries= document.getElementById("countries<?=$val['grafik'].$key2;?>").getContext("2d");
...
<?php  }
else if($val['grafik']=='2'){?>
<div class="grafik_f">
<canvas id="countries<?=$val['grafik'].$key2;?>" width="800" height="300">
      
     <script>
            var income = document.getElementById("countries<?=$val['grafik'].$key2;?>").getContext("2d");
            var barData = {
...
<canvas id="countries<?=$val['grafik'].$key2;?>" width="800" height="300">
     ...
     <script>
        var buyers = document.getElementById('countries<?=$val['grafik'].$key2;?>').getContext('2d');
        var buyerData = {
...
<br>
 <p class="pcolor">Лицензия: <?=$GLOBALS["foton_setting"]["license"];?></p>
      <p class="pcolor">Версия ядра <?=$GLOBALS["foton_setting"]["coref"];?> <? if($this->core->chmod_id([1,1])){?><span class='up_core'>Обновить</span><?}?></p>
      <p class="pcolor"> Login: <?=$_SESSION['login'];?></p>
      <p class="pcolor"> Права: <?=$_SESSION['chmod'];?></p>
...
<?php   foreach($data['spbackup'] as $f){?>
<tr><td><?=$f;?></td><td><a href='/zip/?file=<?=$f;?>' class='href-zip'>Скачать</a> <a href='/zip/?del=<?=$f;?>' class='href-zip'>Удалить</a> </td></tr>
<?php  }?>
</table>
<?php  }?>
<table class='tbl'>
<tr><td>Пароль 1</td><td><?=$controller_class->passints()['pass1'];?></td></tr>
<tr><td>Пароль 2</td><td><?=$controller_class->passints()['pass2'];?></td></tr>
</table>
...
<table class='tbl'>
<?php  $ms=$controller_class->fileinfo('controller',$GLOBALS['foton_setting']['sitedir']);
foreach($ms as $key=>$val){
    echo '<tr><td>Имя файла</td><td id="f-tb">'.$val['name'].'</td></tr>';
echo '<tr><td>Размер файла</td><td>'.ceil($val['size']/1024).'Kb</td></tr>';
echo '<tr><td>Последнее время доступа</td><td>'.date( 'Y-m-d H:i',$val['atime']).'</td></tr>';
...
<table class='tbl'>
<?php  $ms=$controller_class->fileinfo('controller',$GLOBALS['foton_setting']['admindir']);
foreach($ms as $key=>$val){
    echo '<tr><td>Имя файла</td><td id="f-tb">'.$val['name'].'</td></tr>';
echo '<tr><td>Размер файла</td><td>'.ceil($val['size']/1024).'Kb</td></tr>';
echo '<tr><td>Последнее время доступа</td><td>'.date( 'Y-m-d H:i',$val['atime']).'</td></tr>';
...
<table class='tbl'>
<?php  $ms=$controller_class->fileinfo('view/site');
foreach($ms as $key=>$val){
    echo '<tr><td>Имя файла</td><td id="f-tb">'.$val['name'].'</td></tr>';
echo '<tr><td>Размер файла</td><td>'.ceil($val['size']/1024).'Kb</td></tr>';
echo '<tr><td>Последнее время доступа</td><td>'.date( 'Y-m-d H:i',$val['atime']).'</td></tr>';
